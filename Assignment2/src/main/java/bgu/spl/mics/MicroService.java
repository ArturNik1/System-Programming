...
public abstract class MicroService implements Runnable {
    ...
    protected final void terminate() {
        System.out.println("terminate "+ getName());
        msgBus.unregister(this); // TODO check if the unregister is necessary
    ...
    public final void run() {
        ...
        ServiceCounter counter = ServiceCounter.getInstance();
        counter.incServiceCounter();
        int targetCounter = counter.getTargetCounter();
        synchronized (counter)
        {
            if(counter.getServiceCounter()<targetCounter) {
                try {
        ...
        while (!terminated) {
            try {
                ...
                Callback cb = map.get(message.getClass());
                if (cb != null) {
                    cb.call(message);
                }
            } catch (InterruptedException e) {
