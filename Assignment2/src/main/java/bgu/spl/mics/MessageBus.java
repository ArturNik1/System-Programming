...
public interface MessageBus {
     ...
     * @pre: this.isSubscribeEvent(type,m)==false
     * @post: this.isSubscribeEvent(type,m)==true
     */
     ...
     * @param m    	The subscribing micro-service.
     * @pre: this.isRegistered(m)==true
     * @pre: this.isSubscribeBroadcast(type,m)==false
     * @pre: this.isSubscribeBroadcast(type,m)==true
     */
     ...
     * @pre: sendEvent(Event<T> e)
     * @post: Future<T>.get()=result
     */
     ...
     * @post: (V:isSubscribeBroadcast(b,V)==true) isBroadcastBelongTo(V,b)=true
     */
     ...
     *                              to became available.
     * @pre: this.isRegistered(m)==true
     * @post: this.top()!=@pre(this.top())
     * @post: this.size(m)==@pre(this.size(m))-1
     *
     */
    Message awaitMessage(MicroService m) throws InterruptedException;

    boolean isSubscribeBroadcast(Class<? extends Broadcast> type, MicroService m);

    <T> boolean isSubscribeEvent(Class<? extends Event<T>> type, MicroService m);

    /**
     * checks if there is queue for microservice
     * @param microService
     * @return true if there is a queue for microservice
     */
    boolean isRegistered(MicroService microService);

    /**
     *
     * @param m
     * @return first message in queue , null if queue is empty
     *
     */
    Message top(MicroService m);
    //TODO:maybe to add isEmpty Function

    /**
     *
     * @param m
     * @return size of m's queue or -1 if not exist
     */
    int getSize(MicroService m);
    <T>boolean isEventBelongTo(MicroService m,Event<T> event);
}
